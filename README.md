...
Các thành phần hoàn chỉnh từ nhánh `main` được giữ lại theo cùng cấu trúc `source/`:

- `software/mqtt-dashboard`: dashboard + MQTT/HiveMQ bridge + cảnh báo email.
- `hardware/mqtt_main_controller`: firmware bộ điều khiển dùng MQTT tương ứng.
- `hardware/esp32cam_gateway`: firmware camera/stream gọn nhẹ dùng với MQTT dashboard.

Stack HTTP/Prisma ở `software/{frontend,backend}` là profile mặc định. MQTT dashboard
là profile tương thích độc lập và không nên chạy cùng backend mặc định trên port `3001`.
