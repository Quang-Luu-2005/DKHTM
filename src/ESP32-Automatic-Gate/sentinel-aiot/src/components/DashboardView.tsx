...
import type { BoardAction } from "../services/boardApi";
...
interface DashboardViewProps {
  ...
  onCommand: (action: BoardAction) => Promise<void>;
  mqttConnected: boolean;
  logs: AuditLog[];
  onAddLog: (log: Omit<AuditLog, "id" | "timestamp">) => void;
  isEmergencyLocked: boolean;
  faceScanState: "idle" | "loading" | "scanning" | "granted" | "denied" | "error";
  faceScanMessage: string;
}
...
export default function DashboardView({
  ...
  const faceStatusColor = faceScanState === "granted"
    ? "bg-emerald-500"
    : faceScanState === "denied" || faceScanState === "error"
      ? "bg-rose-500"
      : faceScanState === "loading" || faceScanState === "scanning"
        ? "bg-amber-400"
        : "bg-sky-400";
  ...
  const handleForceLockUnlock = async (lock: boolean) => {
    if (servoLoading) return;
    if (!mqttConnected) {
      alert("Sentinel chưa kết nối tới HiveMQ. Hãy kiểm tra MQTT bridge.");
      return;
    }
    setServoLoading(true);
    try {
      await onCommand(lock ? "close" : "open");
      onUpdateHardware({
        ...hardware,
        servoLocked: lock,
      ...
      alert(error instanceof Error ? error.message : "Không gửi được lệnh tới mạch");
    } finally {
  ...
  const handleNormalMode = async () => {
    ...
    try {
      ...
      onAddLog({
        subjectName: "Khôi phục chế độ hoạt động bình thường",
        accessMethod: "Manual Override",
        gateId: "GT-NORTH-01",
        status: "ONLINE",
        confidence: "100%",
      ...
      alert(error instanceof Error ? error.message : "Không khôi phục được chế độ bình thường");
    } finally {
      setServoLoading(false);
    }
  };
  ...
  const handleToggleLights = async () => {
    ...
    if (!mqttConnected) {
      alert("Sentinel chưa kết nối tới HiveMQ.");
      return;
    }
    setLightsLoading(true);
    try {
      const isRed = hardware.indicatorLed.includes("RED");
      await onCommand(isRed ? "led_green" : "led_red");
      onUpdateHardware({
        ...hardware,
        indicatorLed: isRed ? "GREEN / ACCESS ALLOWED" : "RED / RESTRICTED"
      });
    } catch (error) {
      alert(error instanceof Error ? error.message : "Không gửi được lệnh LED");
    } finally {
  ...
  const handleToggleBuzzer = async () => {
    ...
    try {
      const activate = hardware.systemBuzzer !== "ACTIVE";
      await onCommand(activate ? "buzzer_on" : "buzzer_off");
      onUpdateHardware({
        ...hardware,
        systemBuzzer: activate ? "ACTIVE" : "MUTED"
      });
    } catch (error) {
      alert(error instanceof Error ? error.message : "Không gửi được lệnh buzzer");
    } finally {
    ...
    <div className="flex flex-col gap-6">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex shrink-0">
          <div className="flex items-center gap-2 bg-[#111113] px-3.5 py-1.5 rounded-xl border border-[#1E293B]">
            <span className={`w-2 h-2 rounded-full ${mqttConnected ? "bg-[#10B981] animate-pulse" : "bg-rose-500"}`} />
            <span className="font-mono text-[9px] font-semibold text-[#94A3B8] tracking-widest uppercase">
              {mqttConnected ? "HiveMQ đã kết nối" : "HiveMQ mất kết nối"}
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-8 flex flex-col gap-6">
          {/* Real ESP32-CAM MJPEG feed proxied by the Sentinel API. */}
          <div className="relative aspect-video bg-[#111113] rounded-2xl border border-[#1E293B] overflow-hidden shadow-xl group" title="Luồng Camera trực tiếp (ESP32-CAM)">
            ...
            <Esp32CameraStream className="absolute inset-0" />
            ...
            <div className="absolute inset-0 pointer-events-none p-6 flex flex-col justify-between">
              ...
              <div className="flex justify-between items-start">
                <div className="bg-black/85 backdrop-blur-md px-3 py-1.5 border border-[#1E293B] rounded-lg flex items-center gap-2">
                  <span className={`w-1.5 h-1.5 ${faceStatusColor} rounded-full animate-ping shrink-0`} />
              ...
              <div className="absolute top-[30%] left-[35%] w-[30%] h-[40%] border border-dashed border-[#94A3B8]/30 rounded-xl flex items-center justify-center">
                <span className="font-mono text-[8px] text-[#94A3B8] tracking-widest bg-black/75 px-2.5 py-1 rounded border border-[#1E293B] uppercase">
                  {faceScanState === "scanning" || faceScanState === "loading"
              ...
              <div className="flex justify-between items-end">
                <div className="flex flex-col gap-1.5">
                  <div className="bg-black/75 border border-[#1E293B] text-[#94A3B8] text-[8px] font-mono font-medium px-2 py-1 rounded tracking-widest uppercase">
                    CAMERA • GIỮ KHUÔN MẶT RÕ TRONG 2 GIÂY
                  </div>
                  <div className="bg-black/75 border border-[#1E293B] text-[#94A3B8] text-[8px] font-mono font-medium px-2 py-1 rounded tracking-widest uppercase">
                    {faceScanMessage}
                  </div>
              ...
              <div className="flex flex-col gap-3.5 p-4 bg-[#161618] border border-[#1E293B]/60 rounded-xl transition-all hover:border-[#334155]">
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-[#1E293B]/30">
                  ...
                  <button
                    ...
                    disabled={servoLoading || !hardware.servoLocked}
                    className={`flex-grow py-2 rounded-lg font-sans text-[9px] uppercase tracking-wider font-semibold border flex items-center justify-center gap-1.5 transition-all ${
                      !hardware.servoLocked
                        ? "bg-[#111113]/40 border-transparent text-[#475569] cursor-not-allowed"
                        : servoLoading
                ...
                <button
                  ...
                  className="w-full py-2 rounded-lg font-sans text-[9px] uppercase tracking-wider font-semibold border flex items-center justify-center gap-1.5 transition-all bg-[#111113] hover:bg-[#1C1C1F] text-blue-300 border-[#1E293B] hover:border-blue-500/30 cursor-pointer active:scale-[0.98] disabled:text-[#475569] disabled:cursor-wait"
                >
