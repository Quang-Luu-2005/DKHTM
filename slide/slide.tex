...
\begin{frame}{Nhóm đang làm gì?}
  ...
  \begin{itemize}
    ...
    \item Hệ thống web hiển thị Popup cảnh báo thông minh bằng mô hình LLM, tự động phân tích và sinh log mô tả đối tượng vi phạm từ ảnh camera gửi về.
  \end{itemize}
\end{frame}
...
\begin{frame}{Dùng cho việc gì? -- Bối cảnh}
  ...
  \vspace{0.8em}
  \begin{itemize}
    \item Quy mô sử dụng tối đa \textbf{20 người dùng đã đăng ký}.
...
\begin{frame}{Mục tiêu 1 -- Phát hiện và nhận diện người}
  ...
  \vspace{0.8em}
  Hệ thống dùng camera để phát hiện và nhận diện tối đa \textbf{20 người dùng đã đăng ký} trong phạm vi \textbf{$0.5$ m}, với \textbf{độ chính xác định danh trên $95\%$}, \textbf{tỷ lệ nhận nhầm dưới $2\%$} và thời gian phản hồi \textbf{dưới $1.5$ giây}.
\end{frame}
...
\begin{frame}{Mục tiêu 3 -- Xác thực RFID/NFC và mở cửa tự động}
  ...
  \vspace{0.8em}
  Hệ thống dùng RFID/NFC để xác thực người dùng hợp lệ và mở cửa tự động \textbf{dưới $2$ giây}, đồng thời đạt \textbf{độ chính xác xác thực tối thiểu $98\%$} trong điều kiện vận hành bình thường.
\end{frame}
...
\begin{frame}{Danh mục linh kiện dự kiến}
  ...
  \begin{tabular}{p{0.35cm}p{2.45cm}p{5.9cm}p{0.75cm}p{1.35cm}}
    ...
    4 & Micro Servo SG90 & Điều khiển đóng/mở cơ cấu cổng mô phỏng & 1 & 30.000 \\
